...
class Particle:
    ...
    def compute_H(self, lx, ly):
        ...
        # [Hint 1] The parameter "lx,ly" is the location of landmark.
        q = (lx-x)**2 + (ly-y)**2
        H = np.array([[(lx-x)/np.sqrt(q),(ly-y)/np.sqrt(q)],
                      [-(ly-y)/q,(lx-x)/q]])
    ...
    def update_landmark(self, z, lid):
        ...
        if lid not in self.landmarks:
            ...
            sig = Hinv.dot(self.Qt).dot(Hinv.T)
            ...
            H = self.compute_H(mu[0,0], mu[1,0])
            Q = H.dot(sig).dot(H.T) + self.Qt
            K = sig.dot(H.T).dot(np.linalg.inv(Q))
            e = np.array(z) - np.array(z_hat)
            e[1] = normalize_angle(e[1])
            self.landmarks[lid]["mu"] = mu + K.dot(e.reshape((2,1)))
...
def multi_normal(x, mean, cov):
    ...
    denominator = 2 * np.pi * np.sqrt(np.linalg.det(cov))
    N = np.exp((-1/2*err.T).dot(np.linalg.inv(cov)).dot(err)) / denominator
