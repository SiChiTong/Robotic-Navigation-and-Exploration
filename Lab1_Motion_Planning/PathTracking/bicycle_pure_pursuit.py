...
class PurePursuitControl:
    ...
    def feedback(self, state):
        ...
        L_d = self.kp * v + self.Lfc
        
        target = (self.path[min_idx,0], self.path[min_idx,1])
        idx = min_idx
        dist = min_dist
        for i in range(idx+1, self.path.shape[0]):
            near = (self.path[i,0], self.path[i,1])
            new_dist = np.sqrt((near[1]-y)**2 + (near[0]-x)**2)
            if abs(new_dist-L_d) < abs(dist-L_d):
                dist = new_dist
                target = near
            
        alpha = np.arctan2(target[1]-y, target[0]-x) - np.deg2rad(yaw)
        next_delta = np.rad2deg(np.arctan(2 * l * np.sin(alpha) / L_d))       
