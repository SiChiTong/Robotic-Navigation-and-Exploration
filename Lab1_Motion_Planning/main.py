...
def main():
    ...
    while(True):
        ...
        if nav_pos is not None:
            ...
            if nav_pos != target_pos:

                if plan_type == 0:
                    path = planner.planning(start=tuple(map(int,pos[:2])), goal=nav_pos, img=img_, inter=20)
                    path = np.array(cubic_spline_2d(path, interval=1))
                    controller.set_path(path)
                    target_pos = nav_pos
                elif plan_type == 1:
                    path = planner.planning(tuple(map(int,pos[:2])), nav_pos, 30, img_)
            ...
            target_v = 30 if end_dist > 35 else 0
